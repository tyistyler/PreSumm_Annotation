...
class LossComputeBase(nn.Module):
    ...
    def sharded_compute_loss(self, batch, output,
        ...
        shard_state = self._make_shard_state(batch, output)     # shard_state是个字典，记录了output和target
        # shards函数的作用：对shard_state中key对应的value按照shard_size数进行划分，目的是降低每次loss的计算量
...
class LabelSmoothingLoss(nn.Module):
    ...
    def __init__(self, label_smoothing, tgt_vocab_size, ignore_index=-100):
        ...
        self.register_buffer('one_hot', one_hot.unsqueeze(0))     # [1, 30522]
        self.confidence = 1.0 - label_smoothing           # 0.9
    ...
    def forward(self, output, target):
        ...
        [78]
        """
        model_prob = self.one_hot.repeat(target.size(0), 1) # [78, 30522]
        # target.unsqueeze(1)=[78, 1]
        """
        tensor([[ 4717],
        [ 1998],
        [13286],
        ...
        [ 1999],
        """
        # 使正确token对应位置的概率为0.9，其余位置对应概率为smoothing_value
        model_prob.scatter_(1, target.unsqueeze(1), self.confidence)
        # print(model_prob[0][4717])
        # print(sum(model_prob[0]))
...
class NMTLossCompute(LossComputeBase):
    ...
    def _make_shard_state(self, batch, output):
        """
        output=[1, 78, 768]
        batch.tgt[:,1:]=[1, 78]，表示预测的target，位置从BOS后面的一个开始计算
        """
        return {
            "output": output,
            "target": batch.tgt[:,1:],
    ...
    def _compute_loss(self, batch, output, target):
        bottled_output = self._bottle(output)     # [1*78, 768]
        scores = self.generator(bottled_output)    # self.generator也需要更新梯度
        # print("scores={}".format(scores.size()))   # [78, 30522]
        # print("target={}".format(target.size()))   # [1, 78]
        gtruth=target.contiguous().view(-1)       # [78]
    ...
    for k, v in state.items():
        ...
        if v is not None:
            ...
            if isinstance(v, torch.Tensor):
                # print("v={}".format(v.size()))              # [1, 78, 768]
                # torch.split，默认在dim=0维度进行，对batch进行划分
                for v_chunk in torch.split(v, shard_size):
                    # print("v_chunk={}".format(v_chunk.size()))    # [1, 78, 768]
