...
class Translator(object):
    ...
    def _fast_translate_batch(self,
        ...
        # tile->使[6, 512]变成[30, 512]，相当于复制了四份
        ...
        for step in range(max_length):
            ...
            # alive_seq随着step的增加，逐渐增加，每次取最后一个值作为decoder的输入
            decoder_input = alive_seq[:, -1].view(1, -1)    # [1, 30]
            # Decoder forward.
            decoder_input = decoder_input.transpose(0,1)     # [30, 1]
            # decoder_input只是一个token的表示，所以需要dec_states来记录当前token之前那些token的信息，并在self-attention阶段提供辅助计算
            # print(dec_states.cache["layer_0"]["self_keys"].size())  # neural.py的MultiHeadedAttention给dec_states.cache["layer_0"]["self_keys"]赋初值了
            ...
            log_probs += topk_log_probs.view(-1).unsqueeze(1)  # 忽略通过tile函数repeat的那些value, [30, 30522]
            ...
            if(self.args.block_trigram):              # True
                ...
                if(cur_len>3):
                    # print("有长度大于3的alive_seq={}".format(alive_seq.size()))
                    for i in range(alive_seq.size(0)):
                        ...
                        if fail:
                            # 第i行全部复制-10e20
                            # 如i=4时，代表第一句话中的第4个beam无效，此时curr_scores.reshape(-1, beam_size * vocab_size)
                            # 的结果中，第0个batch的后30522个value就不用管了，只在前30522*4个value中，找到概率top5的index
                            curr_scores[i] = -10e20
            ...
            topk_beam_index = topk_ids // vocab_size           # topk_ids落到哪一个beam
            topk_ids = topk_ids.fmod(vocab_size)              # 取余数，得到vocab中的id

            # Map beam_index to batch_index in the flat representation.
            # beam_offset=[0, 5, 10, 15, 20, 25]
            # 将每个topk_id分配到对应的beam中，共30个！
            ...
            select_indices = batch_index.view(-1)         # [30]-->第一个字符得到的indices[0,0,0,0,0,5,5,...,25,25]
            # print("select_indices={}".format(select_indices))
            """
            alive_seq.index_select(0, select_indices)-->[30, 1]
            是指在alive_seq根据select_indices选择对应的行作为新的结果
            """
            # Append last prediction.--实现beam search
            ...
            if is_finished.any():                 # False
                ...
                for i in range(is_finished.size(0)):
                    ...
                    # print("is_finished[i].nonzero()={}".format(is_finished[i].nonzero())) # 返回非空索引
                    # Store finished hypotheses for this batch.
                    for j in finished_hyp:
                        # print("j={}".format(j))   # j=2, b=5, i=5
                        hypotheses[b].append((
                            topk_scores[i, j],
                            predictions[i, j, 1:])) # predictions是预测的结果
                ...
                # 移除finished的句子，加快推理速度，这里需要注意，由于移除了第(假设)m个句子，会导致m后边句子的index发生变化，
                # 因此要利用下边的index_select操作和上边for循环中的b，i，j相互配合，来保证index准确无误。
            ...
            # Reorder states.
            select_indices = batch_index.view(-1)         # topk的index
            # print("select_indices={}".format(select_indices))
