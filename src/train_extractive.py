...
def validate_ext(args, device_id):
    ...
    if (args.test_all):
        ...
        for i, cp in enumerate(cp_files):
            ...
            max_step = xent_lst.index(min(xent_lst))                       # 最小loss的索引
            if (i - max_step > 10):
                break
        xent_lst = sorted(xent_lst, key=lambda x: x[0])[:3]                    # 取前3个最小loss对应文件
        ...
        while (True):
            ...
            cp_files.sort(key=os.path.getmtime)
            if (cp_files):
                cp = cp_files[-1]                                       # 取最后一个文件
                time_of_cp = os.path.getmtime(cp)                       # 返回最后的修改时间
                if (not os.path.getsize(cp) > 0):
                    time.sleep(60)
                    continue
                if (time_of_cp > timestep):
                    timestep = time_of_cp
                    step = int(cp.split('.')[-2].split('_')[-1])        # 50000
