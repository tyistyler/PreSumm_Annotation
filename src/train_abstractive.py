...
def validate_abs(args, device_id):
    ...
    if (args.test_all):
        ...
        for i, cp in enumerate(cp_files):
            ...
            if (i - max_step > 10):
                break
        xent_lst = sorted(xent_lst, key=lambda x: x[0])[:5]           # 取前5个最小loss对应文件
        ...
        while (True):
            ...
            cp_files.sort(key=os.path.getmtime)
            if (cp_files):
                cp = cp_files[-1]                     # ../models/model_step_6000.pt
                ...
                if (time_of_cp > timestep):
                    timestep = time_of_cp
                    step = int(cp.split('.')[-2].split('_')[-1])
                    validate(args, device_id, cp, step)       # 计算loss
                    test_abs(args, device_id, cp, step)
